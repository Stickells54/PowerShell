...
function DownloadCSRTemplate
{
	...
	Invoke-WebRequest -Uri "https://raw.githubusercontent.com/Stickells54/PowerShell/master/Windows%20Tools/csrtemplate.csv" -OutFile "$SaveLocation\csrtemplate.csv"
...
function ImportExport
{
	...
	$CertsLocation = Read-Host -Prompt "Enter the folder where your CER files are stored i.e C:\CER: "
	...
	foreach ($pfxcert in $CertNames)
	{
		...
		#Regenerate PFX from CRT and Key
		Start-Process -FilePath C:\OpenSSL\openssl.exe -ArgumentList "pkcs12 -export -out $ExportLocation\$pfxcert\$pfxcert-PFX.pfx -password pass:$PFXPWD -inkey $ExportLocation\$pfxcert\$pfxcert.key -in $ExportLocation\$pfxcert\$pfxcert.crt " -NoNewWindow
